...
function Graph() {
  const queryURL = `${iex.base_url}/stock/AAPL/chart/1m?token=${iex.api_token}`;
  ...
  async function gatherInfo() {
    ...
    var stockList = ["AAPL","TSLA", "MRNA", "MSFT", "ZM"]
  ...
  function queryByHistorical(symbol) {
    ...
    return axios.get(
      `${iex.base_url}/stock/` + symbol + `/chart/1m?token=${iex.api_token}`
    )
  }
